    ...
    for con in cons:
        if con.get_n_unasgn() == 1:
            ...
            for d in Var.cur_domain():
                ...
                if not con.check(values): #Not satified
                    Var.prune_value(d)
                    prun_keep.append((Var,d))
                Var.unassign()
            if Var.cur_domain_size()==0:
                return False, prun_keep
    return True, prun_keep
        ...
        for var in con.get_scope():
            for d in var.cur_domain():
                if not con.has_support(var, d):
                    var.prune_value(d)
                    prune_kp.append((var,d))
                    if var.cur_domain_size() == 0:
                        GACQ = []
                        return False, prune_kp
                    else:
                        c_prime = csp.get_cons_with_var(var)
                        for item in c_prime:
                            if item not in GACQ:
                                GACQ.append(item)

    return True, prune_kp
